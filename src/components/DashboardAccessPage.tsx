...
export default function DashboardAccessPage() {
  ...
  const handleAccess = async (e: FormEvent) => {
    ...
    try {
      ...
      if (cleanCode === 'HS-DEMO-01') {
        await router.push(`/dashboard/${cleanCode}`);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex flex-col items-center justify-center p-6 text-brand-text">
      <Link href="/" className="fixed top-6 left-6 p-2 bg-brand-surface rounded-xl border border-brand-border text-brand-text-dim hover:text-brand-text transition-all flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
